...
public class BranoDAOImpl implements BranoDAO {
	...
	public void deleteBranoAndUnlinkGeneri(Long idBranoInput) throws Exception {
		entityManager.createNativeQuery("delete from brano_genere c where c.brano_id = ?1")
				.setParameter(1, idBranoInput).executeUpdate();
		entityManager.createNativeQuery("delete from brano c where c.id = ?1").setParameter(1, idBranoInput)
				.executeUpdate();
	}
	...
	public List<Brano> listaBraniConDescrizione() throws Exception {
		return entityManager
				.createQuery("select distinct b from Brano b join b.generi g where length (g.descrizione) > 10",
						Brano.class)
				.getResultList();
	}
