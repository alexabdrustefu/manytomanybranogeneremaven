...
public class BranoServiceImpl implements BranoService {
	...
	@Override
	public void creaECollegaBranoEGenere(Brano branoTransientInstance, Genere genereTransientInstance)
			throws Exception {
		// questo è come una connection
		EntityManager entityManager = EntityManagerUtil.getEntityManager();
		...
		try {
			...
			// CascadeType.PERSIST, CascadeType.MERGE dentro l'owner della relazione (Brano
			// in
			// questo caso)
	...
	@Override
	public List<Brano> cercaPerDescrizione() throws Exception {
		EntityManager entityManager = EntityManagerUtil.getEntityManager();

		try {
			branoDAO.setEntityManager(entityManager);

			return branoDAO.listaBraniConDescrizione();
		} finally {
			EntityManagerUtil.closeEntityManager(entityManager);
		}
	}
